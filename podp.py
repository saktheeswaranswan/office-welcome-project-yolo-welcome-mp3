...
# Open video file (change to 0 for webcam if needed)
...
while cap.isOpened():
    ...
    for result in results:
        ...
        if len(indices) > 0:
            ...
            for i in indices:
                ...
                # Create a subfolder for each class and save cropped image
                ...
                # Trigger alert if a person (usually label 0) is detected
                if label == 0:
                    person_detected = True

                # Prepare detection data for CSV and JSON
    ...
    # Save frame's detection data to JSON results (even if no detection, frame info is stored)
    frame_result = {
        "frame_id": frame_id,
