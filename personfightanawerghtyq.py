...
# CSV columns
columns = ["ID", "Timestamp", "Class", "X1", "Y1", "X2", "Y2", "Confidence", "Image_Path"]
...
# Track last welcome sound play time
last_welcome_time = datetime.now() - timedelta(seconds=3)
...
while cap.isOpened():
    ...
    detections = []  # Store detections in the current frame
    person_detected = False  # Flag for playing sound
    ...
    for result in results:
        ...
        if len(indices) > 0:
            ...
            for i in indices:
                ...
                cv2.putText(frame, f"{class_name} {confidence:.2f} (ID: {detection_id})", 
                            (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
                
                detection_id += 1  # Increment unique detection ID
    ...
    if person_detected and (current_time - last_welcome_time).total_seconds() > 3:
        ...
        time.sleep(1.5)  # Play for 1.5 seconds
        pygame.mixer.music.stop()  # Stop the sound
        last_welcome_time = current_time  # Update last play time
